import argparse

argparser = argparse.ArgumentParser()

# Data paths
argparser.add_argument('--data_dir', type=str,
                      default='data/CMIN-US/price/preprocessed/')
argparser.add_argument('--news_dir', type=str,
                      default='data/CMIN-US/news/preprocessed/')
argparser.add_argument('--dataset_save_dir', type=str,
                      default='dataset/OriginalHAN/')

# Model parameters
argparser.add_argument('--vocab_size', type=int, default=10000)
argparser.add_argument('--embedding_dim', type=int, default=100)
argparser.add_argument('--hidden_dim', type=int, default=128)
argparser.add_argument('--num_class', type=int, default=2)

# Training parameters
argparser.add_argument('--learning_rate', type=float, default=1e-3)
argparser.add_argument('--weight_decay', type=float, default=1e-5)
argparser.add_argument('--batch_size', type=int, default=64)
argparser.add_argument('--dr', type=float, default=0.3)
argparser.add_argument('--train_epochs', type=int, default=10)
argparser.add_argument('--check_interval', type=int, default=1)

# Data dimensions
argparser.add_argument('--days', type=int, default=10)
argparser.add_argument('--max_num_tweets_len', type=int, default=20)
argparser.add_argument('--max_num_tokens_len', type=int, default=30)

# System parameters
argparser.add_argument('--device', type=str, default='cuda')
argparser.add_argument('--num_workers', type=int, default=4)
argparser.add_argument('--pin_memory', type=bool, default=True)
argparser.add_argument('--seed', type=int, default=2024)
argparser.add_argument('--save_dir', type=str, default='checkpoints/')
argparser.add_argument('--optimizer', type=str, default='adam')
argparser.add_argument('--usewb', type=bool, default=False)

# Date ranges
argparser.add_argument('--train_start_date', type=str, default='2018-01-01')
argparser.add_argument('--train_end_date', type=str, default='2021-04-30')
argparser.add_argument('--dev_start_date', type=str, default='2021-05-01')
argparser.add_argument('--dev_end_date', type=str, default='2021-08-31')
argparser.add_argument('--test_start_date', type=str, default='2021-09-01')
argparser.add_argument('--test_end_date', type=str, default='2021-12-31')

# Dataset paths
argparser.add_argument('--train_x_path', type=str,
                      default='dataset/OriginalHAN/train_x.csv')
argparser.add_argument('--train_y_path', type=str,
                      default='dataset/OriginalHAN/train_y.csv')
argparser.add_argument('--dev_x_path', type=str,
                      default='dataset/OriginalHAN/dev_x.csv')
argparser.add_argument('--dev_y_path', type=str,
                      default='dataset/OriginalHAN/dev_y.csv')
argparser.add_argument('--test_x_path', type=str,
                      default='dataset/OriginalHAN/test_x.csv')
argparser.add_argument('--test_y_path', type=str,
                      default='dataset/OriginalHAN/test_y.csv')

# Vocabulary parameters
argparser.add_argument('--min_word_freq', type=int, default=2)
argparser.add_argument('--max_vocab_size', type=int, default=10000)

args = argparser.parse_args()